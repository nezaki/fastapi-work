...
# Fargate(ARM64) 向けにビルド。Colima / Apple Silicon 環境ではそのままネイティブビルド可能。
...
# 同じ latest タグを使う場合は強制的に新しいデプロイを発火
aws ecs update-service --region "$AWS_REGION" \
  --cluster "$PROJECT-cluster" --service "$PROJECT-service" \
  --force-new-deployment
```

> イメージごとにタグを変える運用なら、`ImageTag` を変えて手順3の `aws cloudformation deploy` を再実行します。
...
> 特定ブランチのみ許可したい場合は `GitHubRefPattern="ref:refs/heads/main"` を指定します。
...
| 入力 | 既定 | 説明 |
| --- | --- | --- |
| `image_tag` | `latest` | 付与するタグ。指定タグと commit SHA の2つを push する |
...
2. `image` を今 push したイメージ URI に差し替え
