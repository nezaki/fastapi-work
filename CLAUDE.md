...
アプリは2つ:
- 公開サービス [app/main.py](app/main.py): ALB 配下。`/internal` で内部サービスを呼び、`POST /enqueue` で **SQS** にメッセージを送信する(プロデューサー専用)。
...
# SQS は SQS_QUEUE_URL(+ AWS 認証 / AWS_REGION)を渡したときのみ送信可能。
# 未設定だと POST /enqueue は 503 を返す。
...
- **SQS**: 公開サービスが送信する Standard キュー(`fastapi-ecs-queue`)+ DLQ(`fastapi-ecs-dlq`)。タスクロールに `sqs:SendMessage` のみ付与し、キュー URL は環境変数 `SQS_QUEUE_URL` で渡す。アイドル課金が無いため `Enabled`(停止/起動)の対象外で常設。受信は CLI/コンソールで確認(コンシューマーは未実装)。
